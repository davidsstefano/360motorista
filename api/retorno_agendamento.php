...
$id_motorista = (isset($_GET['id_motorista'])) ? $_GET['id_motorista'] : null; 
...
if(strlen($id_usuario) > 0 && is_numeric($id_usuario)){
    $sql_bind = "agd.id_usuario = :id_usuario";
    $array_filtro_sql =  array('id_usuario' => $id_usuario);
    $tipo_consulta = 'usuario';
}
if(strlen($id_motorista) > 0 && is_numeric($id_motorista)&& $id_motorista != 0 ){
   
    $sql_bind = $sql_bind . ' AND agd.id_motorista = :id_motorista';
    $array_filtro_sql['id_motorista'] = $id_motorista;
...
if(strlen($data_filtro) > 7){
    ...
    if(fncIsDate($data_filtro)){
        ...
        $sql_bind = $sql_bind . ' AND agd.agendamento_data = :date';
...
//verificar se querystring (endereço do navegador) tem o id_usuario. se nao tiver um ou outro, dar erro
